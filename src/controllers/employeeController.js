...
export async function createEmp(req, res) {
    ...
    // ⭐ emp_id is now AUTO-GENERATED by middleware
    const { emp_id, emp_name, org_id } = req.body || {};
    
    // Validation
    if (!emp_id) {
        return badRequest(res, 'Failed to generate emp_id', 'GENERATION_ERROR');
    }
    
    if (!emp_name) {
        return badRequest(res, 'emp_name is required', 'VALIDATION');
    }
    ...
    try {
        ...
        await auditLog({ 
            ...
            req, 
            meta: { emp_id, emp_name, org_id } 
        });
        
        // ⭐ Return the generated ID to user
        return ok(res, { 
