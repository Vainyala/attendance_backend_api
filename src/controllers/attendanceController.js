...
export async function createAtt(req, res) {
    ...
    // ⭐ att_id is AUTO-GENERATED by middleware
    const { 
        att_id, 
        emp_id
    } = req.body || {};
    ...
    // Validation
    if (!att_id) {
        return badRequest(res, 'Failed to generate att_id', 'GENERATION_ERROR');
    }

    if (!emp_id) {
        return badRequest(res, 'emp_id is required', 'VALIDATION');
    }
    ...
    try {
        ...
        await errorLog({ err, req, context: { att_id, emp_id } });
...
export async function getAtt(req, res) {
  ...
  try {
    ...
    //fetch the project_site_name from project_site_mapping using project_id and project_site_id
